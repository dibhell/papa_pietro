...
// Papa Pietro – kalkulator ciasta do pizzy
// Minimalistyczny futurystyczny UI (Model A1)
// Składniki liczone z baker's %, drożdże liczone z profilu TK/TO + temperatury.
...
// Proste "testy" computeYeastPct do ręcznej weryfikacji w konsoli
...
const DoughCalculator: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6">
      <div className="w-full max-w-3xl bg-slate-900/60 backdrop-blur-xl border border-slate-700/60 rounded-2xl p-8 space-y-8 shadow-xl">
        <header className="space-y-1">
          <h1 className="text-2xl font-semibold tracking-wide text-cyan-300">
            Papa Pietro – Pizza Calculator
          </h1>
          <p className="text-sm text-slate-400">
            Wybierz mąkę (w tym marketowe typy), profil TK/TO i pozwól, żeby Papa Pietro wyliczył drożdże i składniki.
          </p>
        </header>
        ...
        <section className="grid md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div>
              ...
              <select
                className="w-full bg-slate-900/40 border border-slate-700 rounded-xl px-3 py-2 text-sm"
                value={flourType}
                onChange={(e) => {
  ...
  <div className="flex-1 h-2 bg-slate-700 rounded-full overflow-hidden">
    <div
      ...
      style={{ width: `${(currentFlour.protein - 8) * 12}%` }}
    />
  </div>
  <span className="text-cyan-300">siła</span>
</div>
<p className="mt-1 text-[11px] text-slate-500">
                Białko: {currentFlour.protein}% (siła mąki) · domyślna hydracja: {currentFlour.hydration}%
              </p>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <input
                  type="number"
                  className="w-full bg-slate-900/40 border border-slate-700 rounded-xl px-3 py-2"
                  value={tkHours}
        ...
        <section className="pt-4 border-t border-slate-700/40">
          ...
          <p className="mt-3 text-[11px] text-slate-500">
            Mąka liczona jest z sumy procentów (1 + hydracja + sól + oliwa + drożdże), a następnie wyprowadzane są
            pozostałe składniki.
          </p>
        </section>
      </div>
